...
"=============================================================================
"Vim:
function! alti_l#lim#misc#expand_keycodes(str) "{{{
  return substitute(a:str, '<\S\{-1,}>', '\=eval(''"\''. submatch(0). ''"'')', 'g')
endfunction
"}}}


function! alti_l#lim#misc#get_cmdresults(cmd) "{{{
...
function! alti_l#lim#misc#get_sid(...) "{{{
  let path = !a:0 ? expand('%:p') : fnamemodify(expand(a:1), ':p')
  let snames = alti_l#lim#misc#get_cmdresults('scriptnames')
...
"}}}
function! alti_l#lim#misc#match_sids(pat) "{{{
  let snames = alti_l#lim#misc#get_cmdresults('scriptnames')
  let sids = []
...
"}}}
function! alti_l#lim#misc#get_scriptpath(sid) "{{{
  let snames = alti_l#lim#misc#get_cmdresults('scriptnames')
  let path = substitute(get(snames, a:sid-1, ''), '^\s*\d\+:\s\+', '', '')
  return path=='' ? '' : fnamemodify(path, ':p')
endfunction
"}}}
function! alti_l#lim#misc#get_scriptinfos(...) "{{{
...
function! alti_l#lim#misc#get_sfuncs(...) "{{{
  let path = a:0 ? expand(a:1) : expand('%')
  let sid = alti_l#lim#misc#get_sid(path)
  if !sid
  ...
  let funcs = alti_l#lim#misc#get_cmdresults('function')
...
function! alti_l#lim#misc#hlecho(fmt, ...) "{{{
  for list in a:000
...
"======================================
"Data:
function! alti_l#lim#misc#uniq(list) "{{{
...
"======================================
"System:
function! alti_l#lim#misc#path_encode(path) "{{{
  return substitute(a:path, '[=:/\\]', '\=get({"=": "==", ":": "=-"}, submatch(0), "=+")', 'g')
endfunction
"}}}
function! alti_l#lim#misc#path_decode(fname) "{{{
  return substitute(a:fname, '==\|=+\|=-', '\={"==": "=", "=-": ":", "=+": "/"}[submatch(0)]', 'g')
endfunction
"}}}


function! alti_l#lim#misc#infer_plugin_pathinfo(path) "{{{
